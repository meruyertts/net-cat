...
This project must work in a similar way that the original NetCat works, in other words, it creates a group chat. The project has the following features :
...
* Messages sent, is identified by the time that was sent and the user name of who sent the message, example : [2020-01-20 15:48:41][client.name]:[client.message]
...
$
```
* The client is answered with a linux logo and ask for their name, when connection is received
```
$ nc $IP $port
Welcome to TCP-Chat!
         _nnnn_
